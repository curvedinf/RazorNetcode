...
export namespace razor {
	...
	unsigned int copyIn(void* data, unsigned int position, auto in_value) {
		*((std::remove_reference_t<decltype(in_value)>*)((uint8_t*)data+position)) = in_value;
	...
	unsigned int copyInArray(void *data, unsigned int position, 
		...
		memcpy((uint8_t*)data+position, in_array, copy_len);
		return copy_len;
	}
	...
	unsigned int copyOutArray(auto* out_array, void *data, 
		...
		memcpy(out_array, (uint8_t*)data+position, copy_len);
		return copy_len;
	}
