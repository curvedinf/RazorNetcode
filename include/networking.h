...
#define RAZOR_CONNECTION_ANY "ANY"
#define RAZOR_PACKET_MAX_SIZE 508
#define RAZOR_PACKET_SEGMENT_MAX_SIZE RAZOR_PACKET_MAX_SIZE-12
#define RAZOR_NETWORKING_MAX_DUPLICATE_WAIT 10 * RAZOR_NANOS_PER_SECOND
...
namespace razor {
	...
	public:
		...
		Connection() {
			this->log_file = NULL;
			this->next_channel = 1;
			this->remote_host_and_port = RAZOR_CONNECTION_ANY;
		}
		...
		bool openSocket(unsigned short port, std::string remote=RAZOR_CONNECTION_ANY) {
			this->port = port;
		...
		bool send(std::string host_and_port, std::string message) {
			...
			for(int i=0; i<size; i+=RAZOR_PACKET_SEGMENT_MAX_SIZE) {
				int part_length;
				if(size-i < RAZOR_PACKET_SEGMENT_MAX_SIZE) {
					part_length = size-i;
				} else {
					part_length = RAZOR_PACKET_SEGMENT_MAX_SIZE;
				}
		...
		bool receive(std::string* host_and_port, std::string* message) {
			...
			Packet p;
			UDPpacket* up = SDLNet_AllocPacket(RAZOR_PACKET_MAX_SIZE);
			...
			while(true) {
				...
				if(this->remote_host_and_port != RAZOR_CONNECTION_ANY &&
